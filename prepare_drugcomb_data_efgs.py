...
# =========================
def Mol2PyGAtomGraph(mol, target_x_dim: int = 78):
    """
...
def creat_data(datafile):
    ...
    with open(datafile, 'rb') as f:
        df = pickle.load(f)
    cell_df = df[["Cell_Line_ID", "CellLine"]].drop_duplicates(subset=["Cell_Line_ID"]).reset_index(drop=True)
    cell_features = pd.DataFrame(cell_df["CellLine"].tolist())
    cell_concat = pd.DataFrame({"features": cell_features.apply(lambda row: np.concatenate(row.values), axis=1)})
    cell_expanded = pd.DataFrame(cell_concat["features"].to_list())
    cell_features1 = pd.concat([cell_df["Cell_Line_ID"], cell_expanded], axis=1).to_numpy()

    # read drug combination data and drug ID comparison table
    drug2smiles_1 = df[["Drug1_ID", "Drug1"]].set_axis(["drug", "smiles"], axis=1)
    drug2smiles_2 = df[["Drug2_ID", "Drug2"]].set_axis(["drug", "smiles"], axis=1)
    drug2smiles = pd.concat([drug2smiles_1, drug2smiles_2],
                            axis=0, ignore_index=True).drop_duplicates(subset=["drug"]).reset_index(drop=True)
    ...
    for smile in compound_iso_smiles:
        ...
        for index, row in drug2smiles.iterrows():
            if row["smiles"] == smile:
                smile_pharm_graph[row["drug"]] = graphs

    drug1, drug2, cell, label = (
        list(df["Drug1_ID"]),
        list(df["Drug2_ID"]),
        list(df["Cell_Line_ID"]),
        list(df["CSS"]),
    )
    drug1, drug2, cell, label = (
        np.asarray(drug1),
...
if __name__ == "__main__":
    datafile = 'datas/DrugComb/drugcomb.pkl'
    creat_data(datafile)
