...
def creat_data():
    ...
    - cellfile: cell line simple feature file (CSV format)
    """
    cell_features = []
    cellfile = 'datas/ALMANAC/cell line_gene_expression.csv'
    ...
    datafile = 'datas/ALMANAC/drug_synergy.csv'
    df = pd.read_csv(datafile)
    smifile = 'datas/ALMANAC/drug_smiles.csv'
    df_id = pd.read_csv(smifile)

    compound_iso_smiles = list(df_id["isosmiles"])
    compound_iso_smiles = set(compound_iso_smiles)

    smile_pharm_graph = {}
    smi2drug = dict(zip(df_id['isosmiles'], df_id['name']))
    ...
    id2drug = dict(zip(df_id['pubchemid'], df_id['name']))
    drug1 = df["drug_a"].map(id2drug).to_numpy()
    drug2 = df["drug_b"].map(id2drug).to_numpy()
    cell = df["cell_line"].to_numpy()
    label = df["synergy"].to_numpy()
